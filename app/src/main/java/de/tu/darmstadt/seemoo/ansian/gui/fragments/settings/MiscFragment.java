...
public class MiscFragment extends MyPreferenceFragment {
    ...
    private static final int TRANSMISSION_FILE_RESULT_CODE = 2;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        findPreference("filesource_file_name").setOnPreferenceClickListener(this);
        findPreference("send_file_name").setOnPreferenceClickListener(this);
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        if (preference.getKey().equals("filesource_file_name")) {
            ...
            return false;
        } else if (preference.getKey().equals("send_file_name")) {
            try {
                Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
                intent.setType("*/*");
                intent.addCategory(Intent.CATEGORY_OPENABLE);
                startActivityForResult(Intent.createChooser(intent, "Select a file (8-bit complex IQ samples)"),
                        TRANSMISSION_FILE_RESULT_CODE);
        ...
        if (data != null) {
            ...
            switch (requestCode) {
                case FILESOURCE_RESULT_CODE:
                    ...
                    break;
                case TRANSMISSION_FILE_RESULT_CODE:
                    if (uri != null) {
                        String filepath = FileUtils.getPath(getActivity(), uri);
                        if (filepath != null) {
