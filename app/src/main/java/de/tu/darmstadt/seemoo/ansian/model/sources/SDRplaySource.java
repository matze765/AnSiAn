...
/**
 * <h1>AnSiAn - sdrplay source</h1>
 *
 * Module: SDRplaySource.java Description: Simple source of IQ sampling by
 * reading from IQ files generated by the SDRplay.
 *
 * @author Dennis Mantz
 *
 ...
 *         02110-1301 USA
 */
public class SDRplaySource implements IQSourceInterface {
	...
	private static final String LOGTAG = "SDRplaySource";
	private static final int QUEUE_SIZE = 20;
	public static final int[] OPTIMAL_SAMPLE_RATES = { 1000000, 2000000, 2048000, 2500000, 5000000, 7500000, 10000000};
	public static final long MIN_FREQUENCY = 100000l;
	public static final long MAX_FREQUENCY = 2000000000l;
	public static final int[] POSSIBLE_GAIN_VALUES = { 0, 9, 14, 27, 37, 77, 87, 125, 144, 157, 166, 197, 207, 229, 254, 280, 297, 328, 338, 364, 372, 386, 402,
	...
	public SDRplaySource(String ip, int port) {
		this.ipAddress = ip;
		this.port = port;
	...
	/**
	 * This will start the SDRplay driver app if ip address is loopback and
	...
	public boolean open(Context context, Callback callback) {
		...
		// connecting to the SDRplay instance, read information and inform the
		// callback handler
	...
	public void setSampleRate(int sampleRate) {
		if (isOpen()) {
			...
			if (!commandThread.executeCommand(commandToByteArray(SDRPLAY_COMMAND_SET_SAMPLERATE, sampleRate))) {
	...
	public void setFrequency(long frequency) {
		...
		if (isOpen()) {
			...
			commandThread.executeFrequencyChangeCommand(
					commandToByteArray(SDRPLAY_COMMAND_SET_FREQUENCY, (int) actualSourceFrequency));
	...
	public int[] getPossibleGainValues() {
		return POSSIBLE_GAIN_VALUES;
	}

	public void setGain(int gain) {
		if (isOpen()) {
			if (!commandThread.executeCommand(commandToByteArray(SDRPLAY_COMMAND_SET_GAIN, gain))) {
	...
	/**
	 * Will pack a SDRplay command into a byte buffer
	 *
	 * @param command	SDRPLAY_COMMAND_*
	 * @param arg		command argument (see rtl_tcp/SDRplay documentation)
	 ...
	 *
	 * @param command	SDRplay_COMMAND_*
	 * @param arg1		first command argument (see rtl_tcp/SDRplay documentation)
	 * @param arg2		second command argument (see rtl_tcp/SDRplay documentation)
	 * @return command buffer
	 */
		...
		public void run() {
			...
			while (!stopRequested) {
				try {
					...
					Log.w(LOGTAG, "ReceiverThread: Hit timeout while waiting on samples via TCP! continue...");
					continue;
				} catch (IOException e) {
	...
	/**
	 * This thread will initiate the connection to the SDRplay instance and then
	...
	private class CommandThread extends Thread {
		...
		/**
		 * Called from run(); will setup the connection to the SDRplay instance
		 */
		private boolean connect(int timeoutMillis) {
			...
			try {
				...
				Log.i(LOGTAG, "CommandThread: (connect) Connected to SDRplay at " + ipAddress + ":" + port);

				// Update source name with the new information:
				name = "SDRplay at " + ipAddress + ":" + port;
				...
				// Set all parameters:
				// Frequency:
				executeCommand(commandToByteArray(SDRPLAY_COMMAND_SET_FREQUENCY, (int) frequency));
				...
				Log.e(LOGTAG, "CommandThread: (connect) Error while connecting to SDRplay://" + ipAddress + ":" + port
						+ " : " + e.getMessage());
			} catch (InterruptedException e) {
		...
		public void run() {
			...
			// Perfom "device open". This means connect to the SDRplay instance;
			// get the information
			if (connect(10000)) { // 10 seconds for the user to accept
									// permission request
				// report that the device is ready:
				callback.onIQSourceReady(SDRplaySource.this);
			} else {
				if (!stopRequested) {
					Log.e(LOGTAG, "CommandThread: (open) connect reported error.");
					reportError("Couldn't connect to SDRplay driver instance");
					stopRequested = true;
				}
			...
			inputStream = null;
			outputStream = null;
			SDRplaySource.this.commandThread = null; // mark this source as
													// 'closed'
	...
	public SourceType getType() {
		return SourceType.SDRPLAY_SOURCE;
	}
