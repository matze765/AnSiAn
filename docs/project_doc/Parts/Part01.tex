...
The software design is done separately in front of every sprint.
...
In order to pass information from the demodulators to the graphical
user interface the EventBus library is used. The architecture is
shown in \autoref{fig:demod_text_eventbus}.

\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{gfx/demod_text_eventbus.png}
	\caption{Schema of the Events passed between demodulation and GUI}
...
Outside the scope of the two sprints, some cleanup tasks were necessary
...
blocking queues that synchronizes the various signal processing threads
and also efficiently manages memory buffers. Unfortunately this
...
EventBus to deliver data and buffers are always allocated freshly
...
app before any optimizations. The \ac{GC} kicks in about 8 times a second
...
As can be seen, the buffers are cycling between the threads. The reusage
